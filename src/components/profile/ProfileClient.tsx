    ...
    <div className='h-full overflow-y-auto'>
      ...
      <div className='px-4 -mt-10 pb-4'>
        <div className='flex items-end justify-between mb-3'>
          ...
          {isOwn ? <button className='text-sm font-semibold px-4 py-2 rounded-xl bg-white/[0.06] border border-white/10'>تعديل الملف</button>
                 : <button onClick={toggleFollow} className={'text-sm font-semibold px-4 py-2 rounded-xl transition-all '+(isFollowing?'bg-white/[0.06] border border-white/10':'gradient-purple text-white')}>{isFollowing?'إلغاء المتابعة':'متابعة'}</button>}
        </div>
        <div className='font-bold text-lg'>{profile.full_name} {profile.is_verified&&<span className='text-purple-400 text-sm'>✓ موثق</span>}</div>
        ...
        <div className='flex border-y border-white/[0.06] my-3'>
          {[['متابع',stats.followers],['متابَع',stats.following],['بث',stats.streams]].map(([l,n])=><div key={l as string} className='flex-1 py-3 text-center border-l border-white/[0.06] last:border-l-0'><div className='text-lg font-bold text-purple-400'>{formatNumber(n as number)}</div><div className='text-[11px] text-[#555]'>{l}</div></div>)}
        </div>
        <h3 className='text-sm font-bold text-[#ddd] mb-3'>البثوث</h3>
        {streams.length===0 ? <div className='text-center py-8 text-[#444] text-sm'>لا توجد بثوث</div> : streams.map(s=>(
          <div key={s.id} onClick={()=>s.status==='live'&&router.push('/live/'+s.id)} className={'flex gap-3 py-3 border-b border-white/[0.05] '+(s.status==='live'?'cursor-pointer':'')}>
            <div className='w-14 h-14 rounded-xl bg-gradient-to-br from-[#1a0d2e] to-[#0d1a2e] flex items-center justify-center text-2xl flex-shrink-0'>📡</div>
            <div className='flex-1 min-w-0'>
              <div className='text-sm font-bold truncate'>{s.title}</div>
              <div className='text-xs text-[#666]'>{formatDate(s.started_at)}</div>
              <div className='flex items-center gap-2 mt-1'>{s.status==='live'?<span className='text-[10px] font-bold bg-red-500/20 text-red-400 border border-red-500/30 px-2 py-0.5 rounded-lg'>🔴 مباشر</span>:<span className='text-[10px] font-bold bg-purple-500/20 text-purple-400 border border-purple-500/30 px-2 py-0.5 rounded-lg'>منتهي</span>}<span className='text-[10px] text-[#555]'>👁 {s.viewer_count||0}</span></div>
            </div>
          </div>
        ))}
      </div>
