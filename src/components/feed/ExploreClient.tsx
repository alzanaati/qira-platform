...
export default function ExploreClient({ initialStreams }: { initialStreams: LiveStream[] }) {
  ...
  return (
    <div className="h-full overflow-y-auto">
      <h2 className="text-base font-bold text-[#ddd] px-4 py-3">استكشف البثوث</h2>
      <div className="relative mx-4 mb-3">
        <input value={search} onChange={e=>setSearch(e.target.value)} placeholder="ابحث عن بث..." className="w-full bg-white/[0.06] border border-white/[0.1] rounded-xl px-4 py-2.5 text-white text-sm outline-none focus:border-purple-500 pr-10 transition-colors" />
        <Search size={17} className="absolute right-3 top-1/2 -translate-y-1/2 text-[#555]" />
      </div>
      <div className="flex gap-2 overflow-x-auto px-4 pb-3" style={{scrollbarWidth:'none'}}>
        {[['all','الكل'],...Object.entries(STREAM_CATEGORIES)].map(([k,v]) => (
      ...
      {filtered.length === 0 ? (
        <div className="flex flex-col items-center gap-3 py-10 text-[#444]"><span className="text-5xl">🔍</span><p>لا توجد نتائج</p></div>
      ) : (
          ...
          {filtered.map(stream => (
            <div key={stream.id} onClick={()=>router.push(`/live/${stream.id}`)} className="bg-white/[0.04] border border-white/[0.08] rounded-2xl overflow-hidden cursor-pointer hover:border-purple-500/30 transition-all">
              <div className="h-28 bg-gradient-to-br from-[#1a0d2e] to-[#0d1a2e] flex items-center justify-center relative">
                <span style={{fontSize:36}}>{Object.entries(STREAM_CATEGORIES).find(([k])=>k===stream.category)?.[1]?.split(' ')[0]||'📡'}</span>
                <div className="absolute top-2 right-2"><span className="text-[10px] font-bold bg-red-500 text-white px-2 py-0.5 rounded-lg flex items-center gap-1"><span className="w-1.5 h-1.5 bg-white rounded-full"/>مباشر</span></div>
              </div>
              <div className="p-2.5">
                ...
                <p className="text-[10px] text-[#555] mt-1">👁 {formatNumber(stream.viewer_count)}</p>
              </div>
            </div>
          ))}
        </div>
